...
export function convertObject(
  ...
  let properties = convertProperties(id, name, object.properties);

  // HACK(bnewbold): prevent "MimeTabs missing TabItem" error for empty-input ops.
  if (
...
export function convertProperty(
  ...
  switch (type) {
    case "string":
      // WARNING: hack for a few defs that don't follow the usual Lexicon shape;
      // force them into unions instead.
      if (
    ...
    case "unknown":
      // How to tell OpenAPI we don't know the type.
      return {};
    default:
...
export function getRefLabel(id: string, ref: string) {
