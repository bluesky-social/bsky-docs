...
:::tip
Learn about how mentions and links work under the hood with the [Link, mentions, and rich-text guide](/docs/about-bluesky-content/post-richtext).
:::
...
Each post contains up to four images, and each image can have its own alt text and is limited to 2 megabytes in size. Image files are *referenced* by posts, but are not actually *included* in the post (eg, using `bytes` with base64 encoding). The image files are first uploaded as "blobs" using `com.atproto.repo.uploadBlob`, which returns a `blob` metadata object, which is then embedded in the post record itself.
