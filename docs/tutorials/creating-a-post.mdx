...
<Tabs groupId="sdk">
  ...
  <TabItem value="python" label="Python">
    ...
    Our goal is to turn the handle (`@atproto.com`) into a mention and the URL (`https://en.wikipedia.org/wiki/CBOR`) into a link. To do that, we grab the starting and ending utf8 code-unit locations of each "facet".
    ...
    We then identify them in the facets array, using the _mention_ and _link_ feature types
